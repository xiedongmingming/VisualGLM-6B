...
def main(args):
    ...
    if args.quant in [4, 8]:
        ...
        model = AutoModel.from_pretrained("THUDM/visualglm-6b", trust_remote_code=True).quantize(
            args.quant
        ).half().cuda()
    ...
    with gr.Blocks(css='style.css') as demo:
        ...
        submitBtn.click(
            predict,
            [user_input, image_path, chatbot, max_length, top_p, temperature, history],
            [chatbot, history],
            show_progress=True
        )
        ...
        image_path.upload(
            predict_new_image,
            [image_path, chatbot, max_length, top_p, temperature],
            [chatbot, history],
            show_progress=True
        )
